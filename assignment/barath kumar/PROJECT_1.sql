...
select p.*,pe.EmailAddress,phone.PhoneNumber,
pt.Name from [Person].Person p inner join 
...
Select sod.* from sales.SalesOrderHeader as soh inner join 
sales.SalesOrderDetail sod
on soh.SalesOrderID=sod.SalesOrderID
where 
month(soh.orderdate)=5 and year(soh.orderdate)=2011

--4. Get the total sales made in May 2011 

select sum(linetotal) as TotalSales from sales.SalesOrderDetail sod 
inner join 
...
select datename(mm,orderdate) as Month_Name,
sum(linetotal) as TotalSales from sales.SalesOrderDetail sod
inner join 
sales.SalesOrderHeader as soh
on soh.SalesOrderID=sod.SalesOrderID
